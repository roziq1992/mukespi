    ...
    <div class="di-card bg-white">
        <div class="card-header-custom">
            <div style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap;">
                <div>
                    <h2>⚡ <?php echo isset($button) ? $button : 'Tambah'; ?> Pemeliharaan Cepat</h2>
                    <p>Buat atau edit pemeliharaan cepat untuk inventaris</p>
                </div>
                <a href="<?php echo site_url('data_inventaris/pemeliharaan'); ?>" class="btn btn-light btn-sm">← Kembali</a>
            </div>
        </div>
        ...
        <div class="di-body">
            ...
            <form action="<?php echo isset($action) ? $action : site_url('data_inventaris/pemeliharaan_action'); ?>" method="post" id="formPemeliharaan">
                ...
                <input type="hidden" name="id_pemeliharaan" value="<?php echo isset($id_pemeliharaan) ? $id_pemeliharaan : ''; ?>" />

                <div class="di-actions">
                    <button type="submit" class="btn btn-primary"><?php echo isset($button) ? $button : 'Simpan'; ?></button>
                    <a href="<?php echo site_url('data_inventaris/pemeliharaan'); ?>" class="btn btn-outline-secondary">Batal</a>
                    <?php if (isset($id_inven) && $id_inven != ''): ?>
                        <a href="<?php echo site_url('data_inventaris/maintenance_dashboard'); ?>" class="btn btn-outline-info">📊 Lihat Tracking</a>
                    <?php endif; ?>
                </div>
            </form>
        </div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    document.getElementById('formPemeliharaan').addEventListener('submit', function (e) {
        if (!hiddenId.value) {
            e.preventDefault();
            setScanStatus('', '');
    ...
    // Auto select inventaris jika ada parameter dari URL (fitur lama tetap dipertahankan)
    var urlParams = new URLSearchParams(window.location.search);
    var invenId = urlParams.get('inven_id');
    if (invenId) {
        var fromUrl = findById(invenId);
        if (fromUrl) selectItem(fromUrl);
    }
});

console.log('Pemeliharaan Form loaded successfully');
</script>